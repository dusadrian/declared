...
the argument `na.rm = TRUE`{.R} if the result is `NA`{.R} because of the
...
declared missing values are stored (hence interpreted as) regular `NA`{.R}
missing values in R.
...
need to convert user-defined missing values to regular `NA`{.R} values since
...
necessary, and, despite being stored as an `NA`{.R} value, the value `99` is not
...
because activating the argument `na.rm`{.R} will return the correct result.
...
of potentially problematic `NA`{.R} values.
...
The genuinely problematic values are the empty `NA`{.R} values, and the custom
...
treating existing values as missing, the **declared** package interprets missing
values as existing.

It does so by storing an additional attribute containing the positions (indexes)
of the regular `NA`{.R} values in the object, which should be treated as
