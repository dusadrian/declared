...
Values are printed by default for declared objects, so the underlying codes
remain visible alongside their labels.
...
In the same spirit, the package provides `wmean()`{.R}, `wmedian()`{.R},
`wmode()`{.R}, `wvar()`{.R}, `wsd()`{.R}, `wquantile()`{.R}, `wIQR()`{.R},
`wfivenum()`{.R}, `wstandardize()`{.R}, and `wsummary()`{.R}. Multiple measures
for one or more variables can be requested together with `wmeasures()`{.R}:
...
#> Children 1234   2.426   1.670
```

These functions are adapted to declared objects. For the functions that have
an `na.rm` argument, declared missing values are automatically excluded and
`na.rm` controls the treatment of empty missing values; see the individual help
pages for their defaults.
...
It extends base R and opens up new data analysis possibilities. Its generic
constructors and coercion helpers allow other packages to add methods for new
object types, both for creation and coercion to class `"declared"`{.R}.
