...
test_that("wsd() works", {
  expect_equal(wsd(x), sd(c(1:5, NA), na.rm = TRUE))
})
...
test_that("tests have the same output", {
  expect_snapshot(x)
  expect_snapshot(wsd(x))
  expect_snapshot(using(DF, wsd(Age)))
  expect_snapshot(using(DF, wsd(Age, wt = fweight)))
  expect_snapshot(using(DF, wsd(Age), split.by = Gender))
  expect_snapshot(using(DF, wsd(Age, wt = fweight), split.by = Gender))
  expect_snapshot(using(DF, wsd(Age), split.by = Gender & Children))
  expect_snapshot(
