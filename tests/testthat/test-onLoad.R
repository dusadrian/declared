...
test_that("onLoad functions work", {
  ...
  test <- capture.output(rbind(dfd, dfd))
  expect_error(
    rbind(dfd, dfd[, -4]),
  ...
  test <- capture.output(rbind(dfd, dfd2))
  test <- capture.output(rbind(as.list(dfd), as.list(dfd)))

  test <- capture.output(rbind(dfd, dfd[-seq(1, nrow(dfd)), , drop = FALSE]))
  test <- capture.output(rbind(dfd[-seq(1, nrow(dfd)), , drop = FALSE], dfd[-seq(1, nrow(dfd)), , drop = FALSE]))
  test <- capture.output(rbind(dfd1 = dfd, dfd2 = dfd))
  ldfd <- as.list(dfd)
  test <- capture.output(rbind(dfd, ldfd))
  test <- capture.output(rbind(dfd, as.list(dfd[1, , drop = FALSE])))
  ...
  test <- capture.output(rbind(dfd2, dfd))
  test <- capture.output(rbind(as.matrix(dfd), as.matrix(dfd)))

  test <- capture.output(rbind(dfd, dfd, factor.exclude = FALSE))
  test <- capture.output(rbind(dfd[, 1:2], as.matrix(dfd[, 1:2])))
  test <- capture.output(rbind(dfd, dfd, make.row.names = FALSE))
  test <- capture.output(rbind(dfd, dfd[, 4:1]))
  dfd2 <- dfd
  names(dfd2)[4] <- "fo2"
  ...
  test <- capture.output(order(x))

  # examples from base ?order
  ii <- order(x <- c(1,1,3:1,1:4,3), y <- c(9,9:1), z <- c(2,1:9))
  test <- capture.output(rbind(x, y, z)[,ii])
  test <- capture.output(rbind(x, y, z)[, order(x, -y, z)])
  cy <- as.character(y)
  test <- capture.output(rbind(x, y, z)[, order(x, -xtfrm(cy), z)])
  test <- capture.output(rbind(x, y, z)[, order(x, cy, z, decreasing = c(FALSE, TRUE, FALSE), method="radix")])
  dd <- transform(data.frame(x, y, z), z = factor(z, labels = LETTERS[9:1]))
  test <- capture.output(dd[ order(x, -y, z), ])
  test <- capture.output(dd[ do.call(order, dd), ])
  ...
  test <- capture.output(d4s[ rbind(i, i+1), ])
  x <- c(5:1, 6:8, 12:9)
  y <- (x - 5)^2
  o <- order(x)
  test <- capture.output(rbind(x[o], y[o]))

  # examples from base ?rbind
  dd <- 10
  test <- capture.output(rbind(1:4, c = 2, "a++" = 10, dd, deparse.level = 0))
  test <- capture.output(rbind(1:4, c = 2, "a++" = 10, dd, deparse.level = 1))
  test <- capture.output(rbind(1:4, c = 2, "a++" = 10, dd, deparse.level = 2))
