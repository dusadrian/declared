...
test_that("likely_type() works", {
  ...
  expect_equal(likely_type(as.complex(1)), NULL)
})


test_that("check_measurement() works", {
  expect_equal(check_measurement(NULL), NULL)
...
test_that("internal non exported functions work", {
  ...
  expect_equal(possibleNumeric_(rep(NA, 5)), FALSE)
  
  expect_equal(possibleNumeric_(rep(NA, 5), each = TRUE), as.logical(rep(NA, 5)))
  
  expect_equal(possibleNumeric_(rep(TRUE, 5)), FALSE)
  ...
  expect_equal(wholeNumeric_(rep(NA, 2)), FALSE)

  expect_equal(wholeNumeric_(rep(TRUE, 2)), FALSE)
